...
def get_asn_for_gene(protein_code):
	connection = urllib.urlopen('http://rest.kegg.jp/find/genes/' + str(protein_code))
...
def get_data_from_pathway(asn_pathway_code):
	connection = urllib.urlopen('http://rest.kegg.jp/get/path:' + asn_pathway_code)
	collected_data = {}
	collected_data['name'] = get_line(connection, 'NAME')
	collected_data['class'] = get_line(connection, 'CLASS')
	collected_data['publications'] = []
